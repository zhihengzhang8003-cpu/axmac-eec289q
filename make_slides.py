"""
make_slides.py  --  generate 11-slide defense presentation (~10 min)

Run:  py -3.14 make_slides.py
Out:  AxMAC_Defense_Slides.pptx  (project root)
"""
...
OUT     = os.path.join(ROOT, "AxMAC_Defense_Slides.pptx")
...
def title_bar(slide, title_text, subtitle_text=""):
    """Draw the top title bar common to all content slides."""
...
def bullet_block(slide, items, left, top, width, height,
    ...
    """Render a list of (text, indent_level) tuples as bullet points."""
    y = top
...
txt(s, "Bias-Aware Approximate MAC Design",
    0.6, 1.5, 12.0, 1.0, size=36, bold=True, color=WHITE,
    align=PP_ALIGN.CENTER)
txt(s, "for Energy-Efficient DNN Inference",
    0.6, 2.4, 12.0, 0.8, size=30, bold=True, color=ACCENT,
    align=PP_ALIGN.CENTER)

txt(s, "Jiabo Zhang  ·  Yuxuan Wang",
    0.6, 3.5, 12.0, 0.5, size=18, color=MUTED, align=PP_ALIGN.CENTER)
txt(s, "EEC 289Q — Deep Learning Hardware  |  Spring 2026  |  UC Davis",
    0.6, 4.0, 12.0, 0.5, size=15, color=MUTED, align=PP_ALIGN.CENTER)
...
for label, col in flow:
    w_item = 1.1 if label != "→" else 0.3
    if label == "→":
        txt(s, label, x, 5.1, w_item, 0.5, size=18, color=MUTED,
            align=PP_ALIGN.CENTER)
    else:
        card(s, x, 4.9, w_item, 0.9, fill=SURFACE, border=col)
        txt(s, label, x+0.05, 4.95, w_item-0.1, 0.8, size=11,
            color=col, align=PP_ALIGN.CENTER, bold=True)
    x += w_item + 0.05
...
ops = [
    ("INT8  multiply",  "0.2 pJ",  WHITE),
    ("INT8  add",       "0.03 pJ", WHITE),
    ("FP16  multiply",  "1.1 pJ",  YELLOW),
    ("FP32  multiply",  "3.7 pJ",  RED),
    ("FP8   multiply",  "~0.35 pJ",GREEN),
]
y = 1.8
for op, val, col in ops:
    txt(s, op,  0.55, y, 3.2, 0.42, size=15, color=MUTED)
    txt(s, val, 3.8,  y, 1.8, 0.42, size=15, bold=True, color=col)
    y += 0.45

txt(s, "Source: Horowitz, ISSCC 2014",
    0.5, 4.6, 5.3, 0.35, size=11, italic=True, color=MUTED)

txt(s, "A modern LLM runs 10¹² MACs per token.",
    0.5, 5.1, 5.3, 0.45, size=14, color=WHITE)
txt(s, "Reducing per-MAC energy by even 20%\nsaves terajoules at data-center scale.",
    0.5, 5.6, 5.3, 0.7, size=13, color=MUTED)

# Right: approximation idea
card(s, 6.4, 1.15, 6.6, 5.9)
txt(s, "Truncated Multiplier Idea",
    6.6, 1.25, 6.2, 0.45, size=14, bold=True, color=ACCENT)
txt(s,
    "Drop the lowest K bits of each\n"
    "integer product before accumulation.\n\n"
    "Hardware savings:\n"
    "  Partial-product matrix shrinks by\n"
    "  K(2n–K)/(2n²)  ≈  25% for K=4\n\n"
    "The problem:\n"
    "  Truncation always rounds toward –∞\n"
    "  Error ∈ [0, 2^K)  →  one-sided bias\n"
    "  Over N MACs, bias grows like N  ⚠",
    6.6, 1.8, 6.2, 4.8, size=14, color=WHITE)
...
nums = [
    ("trunc  bias",      "26,617", RED),
    ("round  bias",      "−14",    GREEN),
    ("stochastic  bias", "14",     GREEN),
]
y = 1.75
for label, val, col in nums:
    txt(s, label, 6.9, y, 3.5, 0.48, size=14, color=MUTED)
    txt(s, val,   10.5, y, 2.2, 0.48, size=16, bold=True, color=col)
    y += 0.52
txt(s, "trunc grows ∝ N;  round/stoch grow ∝ √N",
    6.9, 3.15, 5.9, 0.45, size=13, italic=True, color=YELLOW)

# Theorem 1 card
card(s, 6.7, 4.05, 6.3, 3.0)
txt(s, "Theorem 1  (Formal Proof)",
    6.9, 4.15, 5.9, 0.45, size=13, bold=True, color=ACCENT)
thm_lines = [
    "E[Σ εᵢ] = N · 2^(K−1)    [trunc, linear in N]",
    "E[Σ δᵢ] = 0  for any N   [round, zero bias]",
    "Var[Σ] = N · (2^K)² / 12  [same for both]",
]
y = 4.68
for line in thm_lines:
    txt(s, line, 6.9, y, 5.9, 0.48, size=12, color=WHITE)
    y += 0.5
txt(s, "round mode = free stochastic rounding (no RNG needed)",
    6.9, 6.38, 5.9, 0.55, size=12, bold=True, color=GREEN)
