...
| 1 | mac_unit.v + tb_mac_unit.sv (bit-exact trunc/round + stochastic mean check) | RTL/tb written; needs Icarus to execute |
| 2 | aca_adder.v + tb_aca_adder.sv (W=4/8/16/32 across 32-bit) | RTL/tb written; needs Icarus to execute |
| 3 | mac_array.v + tb_mac_array.sv (4x4 output-stationary, inline reference) | RTL/tb written; needs Icarus to execute |
| 4 | mlp_top.v + tb_mlp_top.sv (FSM + tiling, toy 64->16->10) | TODO |
| 5a | Vivado synth + impl + SAIF + Power Report on xc7a100t | TODO (waits for Vivado install) |
| 5b | Quartus synth + .sof + PowerPlay on EP4CE10 + 野火征途 burn | TODO (waits for Quartus install + 野火 .qsf) |
